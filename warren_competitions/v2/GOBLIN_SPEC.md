# Game Spec — Goblin: Good Manners Win
...
**What Goblin borrows:**
- **The Amplify mechanic.** Goblin can broadcast interactions to nearby NPCs. What gets amplified shapes the world — cooperation spreads cooperation; conflict spreads conflict.
...
- **Simple mechanics, deep message.** One verb (take a photo) carries the entire game. Goblin aims for the same economy: a few social actions, a universe of consequences.

**What Goblin does NOT borrow:**
- The 5-minute length (Goblin is replayable, 30+ minutes across levels)
- The inevitably dark ending (Goblin proves cooperation *can* win)
- The passive-observer framing (Goblin is an active participant, not just a camera)
...
**What Goblin borrows:**
- **Named NPC strategies** as literal game characters. Each NPC type in Goblin IS one of these strategies, behaving exactly as game theory predicts.
...
**What Goblin does NOT borrow:**
- The slideshow/essay format (Goblin is a game you play, not a lesson you read)
- The abstract circles (Goblin has a character, a world, and levels)

### Pac-Man (Namco, 1980)

The game should feel linear "like Pac-Man." What that means for Goblin:
...
- **Simple controls, deep strategy.** Pac-Man has one verb (move) and four enemies with distinct AI behaviors. Goblin has four verbs (Wave, Share, Shield, Amplify) and six NPC types with distinct game-theory behaviors.
- **Scoring and grades.** Pac-Man has a high score. Goblin has per-level grades (S / A / B / C / D) for replayability.
...
- Has a small **Patience meter** — if Goblin gets yelled at by hostile NPCs too many times without shielding, Goblin gets "frustrated" and can't act for 2 seconds (this is the penalty, not death)
...
| **Shield** 🛡️ | Q | Blocks a hostile interaction directed at Goblin or a nearby NPC | Prevents trust loss; small +trust with the shielded NPC |
...
Each NPC has a **strategy type** that governs how it reacts to Goblin and to other NPCs. These are directly from game-theory research:
...
- Goblin character with movement + 4 social actions
...
Scripts/
├── Player/
│   ├── GoblinController.cs       (movement + action input)
│   └── GoblinActions.cs          (Wave, Share, Shield, Amplify logic)
├── NPC/
...
- [ ] Goblin sprite placeholder (colored circle with eyes — replace with real art later)
- [ ] GoblinController.cs — WASD movement on a simple arena (single-screen, camera locked)
...
**The deliverable of pre-work is:** Goblin moves around an arena with 4 wandering placeholder NPCs. Nothing interactive yet — that's Sprint 1's job.
...
**Game-theory moment (new for Goblin):** At the end of each sprint, Jorge spends 2 minutes explaining the real-world concept behind what you just built. This is the *preceptorship* version of the end-of-level cards — you understand the theory because you built the mechanic.
...
**Done when:** Goblin moves, waves at Friendly NPCs, trust rises, level-complete screen appears at 100, committed to Git.
...
> Create a new NPC type: `CopycatNPC`. The Copycat starts neutral (yellow tint). When Goblin waves at a Copycat, it turns friendly (green) and waves back (+5 trust). If Goblin bumps into a Copycat without waving (walks into their collider without pressing Spacebar), the Copycat turns unfriendly (orange) for 5 seconds and nearby trust drops −3. Copycats remember Goblin's *last interaction* with them and mirror it — if Goblin's last action was friendly, they stay friendly; if it was a bump, they stay unfriendly until Goblin waves again.
>
...
**Teachable moments to plant:**
- AI will likely make Copycats respond to every interaction globally instead of tracking per-NPC history. Senior asks: *"What if Goblin waves at Copycat A but bumps Copycat B? Should B turn unfriendly?"*
...
> Create `GrudgerNPC`. The Grudger starts cooperative (green). If Goblin *ever* bumps them or fails to shield them from a hostile interaction, the Grudger turns permanently hostile (red) and cannot be converted back. The Grudger's hostility lowers trust of nearby NPCs by −2 every 5 seconds.
>
> Add the Shield action: pressing Q creates a brief shield bubble around Goblin (1-second duration). If a hostile NPC's "yell" hits the bubble instead of Goblin or a friendly NPC, the yell is absorbed — no trust loss. Successfully shielding gives +3 trust.
>
...
- AI may not handle the "fail to shield" condition (Grudger turns hostile if a Hostile NPC yells at them and Goblin doesn't shield in time). Complex conditional. Expected break.
...
>
> **Critical new system: NPC-to-NPC interactions.** Until now, only Goblin's actions affected NPCs. Now, NPCs interact with each other:
...
- NPC-to-NPC interactions create emergent behavior. Junior will see Copycats start cooperating with each other *without Goblin's help*. Senior: *"You just built emergence. The system is doing something you didn't explicitly program."*
...
> Add the Amplify action: pressing F triggers a pulse that broadcasts the most recent interaction within Goblin's radius to all NPCs in a larger radius.
>
...
**Never cut:** Goblin movement, Wave + Share + Shield + Amplify, 5 levels, Friendly + Copycat + Grudger + Hostile NPCs, Trust Meter, level-complete screen, WebGL build, itch.io upload.
...
| Scope creep ("can Goblin have a sword?") | Certain | Point at locked scope in section 3. The whole point is that Goblin DOESN'T have a sword. Add ideas to `BACKLOG.md`. |
...
|---|---|---|---|---|
| **Future Game Designers** (NYU Game Center) | Free 14-week spring workshop on Saturdays. HS freshmen, sophomores, juniors. Learn game design from NYU Game Center faculty. | Spring semester (apply in fall) | **FREE** | **Yes — spring 2027** (9th-grade freshman). Apply fall 2026 WITH Goblin as portfolio. |
| **Tisch Summer HS Game Design** | 4-week residential. 3 college-level courses. Earn 6 NYU credits. Rising juniors/seniors only. | July–Aug (apply Dec of prior year) | ~$7,000+ (financial aid available) | **Summer 2028** at earliest (rising junior). Goblin + Future Game Designers on portfolio = strong application. |

**NYU action items:**
1. **Now:** Write down what games impressed you at the showcase and why. Use as design reference.
2. **Fall 2026:** Apply for Future Game Designers with Goblin as the portfolio piece.
...
| **MIT THINK** | 9th grade (Nov 2026 → Jan 2027). Reframe Goblin as research: *"Can game-theory-based cooperative gameplay mechanics shift player behavior toward pro-social strategies?"* |

### itch.io Game Jam approach

itch.io hosts hundreds of jams per year. Strategy:
1. Publish Goblin as a standalone page (free, permanent URL)
...
- **Goblin** is a new, separate project and repo
- Shared DNA (trust mechanics, NPCs, cooperation themes) but different genres:
  ...
  - Goblin = arcade-strategy with game-theory mechanics (real-time levels, Pac-Man-style progression)
...
1. **What does Goblin look like?** Sketch the character. The character IS the brand.
...
4. **Background narrative?** Option A: no story, pure arcade. Option B: one-sentence frame — *"Goblin's village lost trust. Help rebuild it."* Recommend B — strengthens the CAC personal-story angle without adding writing scope.
...
- **Future Game Designers application (fall 2026):** Apply to NYU's free spring program with Goblin as portfolio
...
- **G4C Student Challenge (March 2027):** Submit Goblin with designer statement emphasizing mechanics-as-message
