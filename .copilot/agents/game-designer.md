...
- Camera & Controls: implement camera that follows hero but allows zoom-out to tactical view; keyboard/mouse controls for selection and issuing orders; toggle mode.
...
**Design Notes — Camera & Player Experience:**
- Camera defaults to a third-person chase of the Chieftain when a single hero is selected. When multiple squads selected or player toggles Tactical Mode, camera pulls up and back to a strategic angle and enables click-drag selection.
...
**Design Notes — Combat & Siege:**
- Fortifications are layered: Outer wall -> Gate -> Courtyard -> Keep. Vertical slice includes only Outer wall & Gate + Tower.
...
**AI Priorities & Behavior:**
- Towers prioritize high-threat targets (siege tools, chieftain, grouped units). Crossbowmen prioritize visible clusters.
...
**Missions & Objectives (examples):**
- Primary: Destroy Gate -> Eliminate Garrison Captain -> Secure Vault (loot). Secondary: Sabotage Beacon (reduces reinforcement calls), Capture Banner (morale boost).

**Progression & Meta:**
- Loot grants recruit points and materials; recruit points are used immediately between missions to hire more goblins or buy single-use siege items (explosive barrel, extra sapper charge).
...
- Audio: short cues for alarm, breach, wall fall, vault open, recruitment, and action confirms. Hand off an audio brief to `audio-game-designer`.

**Acceptance Criteria (vertical slice):**
- Player can select squads and issue orders in Tactical Mode.
- Camera supports third-person follow & tactical zoom with smooth transitions.
...
**Open Questions for you (approve/adjust):**
- Do you want persistent metagame progression between missions (unlock tribes/units), or per-run only? (Recommend: persistent for campaign.)
- Tone: How faithful to Tolkien-like Middle-earth aesthetics? Strict homage might raise IP concerns; recommend 'inspired-by' medieval fantasy.
- Multiplayer: Do you want co-op raids later? (Recommend: single-player for vertical slice.)

**Next steps after approval:**
1. Sign off spec — I will convert major systems into implementation tasks for the build agent.
2. Priority dev order: Camera & controls → Unit framework → Siege system (gate + sapper) → Human AI → UI → Art integration.
3. Create a short audio brief and unit stat CSV for tuning.

---

If this draft matches your intent, mark which items to change (scope, units, camera behavior, or progression) and I'll update the spec and create the task list for implementation.
